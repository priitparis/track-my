...
- `?sheet=Events` — the [event log scraper](../automatic/scraper/events/)'s tab (each point's `properties` also includes `event`, `port`, `country`, `speed`, `course` when present, on top of the usual `time`)
